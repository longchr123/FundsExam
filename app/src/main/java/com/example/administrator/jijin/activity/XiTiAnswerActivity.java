...
import com.example.administrator.jijin.adapter.AnswerVpAdapter;
import com.example.administrator.jijin.bean.TestItem;
import com.example.administrator.jijin.fragment.XiTiTestFragment;
...
public class XiTiAnswerActivity extends FragmentActivity implements View.OnClickListener, ViewPager.OnPageChangeListener, XiTiTestFragment.FragmentCallBack {
    private ImageView iv_back, iv_last, iv_share, iv_save, iv_show, iv_next;
    private TextView tv_current,tv_title;
    ...
    private int tablePosition;//第几个数据库，保存到第几题
    private int listItemSize=0;
    public int firstInPosition=0;//首次进入界面开始答题的位置,用于XiTiTestFragment中。
    ...
    private void initData() {
        ...
        tv_current.setText(currentPosition + "/"+listItemSize);
        ...
        if (cursor.moveToFirst()) {
            tablePosition = cursor.getInt(cursor.getColumnIndex("read"));
        }
        cursor.close();
        if(tablePosition==0)tablePosition=1;
        new AlertDialog.Builder(this).setTitle("温馨提示").setMessage("您上次做到第" + tablePosition + "题,继续上次答题？").
                setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        firstInPosition=vp.getCurrentItem();//获取首次进入界面开始答题的位置。
                    }
    ...
    protected void onPause() {
        ...
        ContentValues values=new ContentValues();
        values.put("read", currentPosition);
        sqLiteDatabase.update("tableNames", values, "EnglishName=?", new String[]{tableName});
    }
